...
public class OAuthUserStoreConfigListenerImpl implements UserStoreConfigListener {
    ...
    public void onUserStoreNamePreUpdate(int tenantId, String currentUserStoreName, String newUserStoreName) throws UserStoreException {
        ...
        try {
            tokenMgtDAO.renameUserStoreNameInAccessTokenTable(tenantId, currentUserStoreName, newUserStoreName);
            tokenMgtDAO.renameUserStoreNameInAuthorizationCodeTable(tenantId, currentUserStoreName, newUserStoreName);
    ...
    public void onUserStorePreDelete(int tenantId, String userStoreName) throws UserStoreException {
        TokenMgtDAO tokenMgtDAO = new TokenMgtDAO();
        try {
            tokenMgtDAO.revokeAccessTokensOfUserStore(tenantId, userStoreName);
            tokenMgtDAO.revokeAuthorizationCodesOfUserStore(tenantId, userStoreName);
        } catch (IdentityOAuth2Exception e) {
